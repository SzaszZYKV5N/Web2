...
Class Menu {
    ...
    public static function getMenu($sItems) {
        ...
        {
            if($menuitem[1] == "")
            { $menu.= "<li class='active'><a href='".SITE_ROOT.$menuindex."' ".($menuindex==$sItems[0]? "class='selected'":"").">".$menuitem[0]."</a></li>"; }
            else if($menuitem[1] == $sItems[0])
            { $submenu .= "<li class='active'><a href='".SITE_ROOT.$sItems[0]."_".$menuindex."' ".($menuindex==$sItems[1]? "class='selected'":"").">".$menuitem[0]."</a></li>"; }
        }
        $menu.="</ul>";
